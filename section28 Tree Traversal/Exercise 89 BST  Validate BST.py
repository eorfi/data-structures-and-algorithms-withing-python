...
class BinarySearchTree:
    ...
    def is_valid_bst(self):
        node_values = self.dfs_in_order()
        for i in range(1, len(node_values)):
            if node_values[i] <= node_values[i-1]: # in this line we are checking if the current node value is less than or equal to the previous node value, if it is then we know that the binary search tree is not valid and we can return False
                return False
        return True
