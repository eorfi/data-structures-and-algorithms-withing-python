...
class LinkedList:
    ...
    # def bubble_sort(self):
    #     if self.length < 2: # this if statement means that if the linked list has less than 2 elements, it is already sorted, so we can return immediately without doing any sorting.
    #         return

    #     sorted_until = None # that mean we will keep track of the portion of the list that is already sorted. Initially, we set it to None, which means that no part of the list is sorted yet.

    #     while sorted_until != self.head.next: # this while loop continues until the sorted portion of the list reaches the second node (self.head.next). this condition ensures that we keep iterating through the list until the entire list is sorted.
    #         current = self.head # we start from the head of the list for each pass through the unsorted portion.
    #         while current.next != sorted_until: # this inner while loop iterates through the unsorted portion of the list, comparing adjacent nodes and swapping their values if they are in the wrong order. The loop continues until we reach the sorted portion of the list (sorted_until).
    #             next_node = current.next # we get the next node to compare with the current node.
    #             if current.value > next_node.value: # if the value of the current node is greater than the value of the next node, we swap their values.
    #                 current.value, next_node.value = next_node.value, current.value
    #             current = current.next # move to the next node in the unsorted portion of the list.
